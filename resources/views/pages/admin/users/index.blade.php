...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
    ...
    <a href="#" onclick="event.preventDefault(); new_user_modal.showModal()" class="btn btn-primary shadow-lg shadow-primary/20 rounded-2xl">
...
<dialog id="new_user_modal" class="modal modal-bottom sm:modal-middle transition-all duration-500">
    <div class="modal-box p-0 max-w-2xl bg-base-100 rounded-3xl overflow-hidden flex flex-col max-h-[90vh] shadow-2xl border border-base-content/5 transform transition-all duration-300 scale-95 opacity-0 [.modal[open]_&]:scale-100 [.modal[open]_&]:opacity-100">
        ...
        <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="flex-1 overflow-y-auto p-8 pt-6 space-y-8 custom-scrollbar scroll-smooth">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div class="form-control group">
                        ...
                        <select name="status" class="select w-full bg-base-200/50 border-none rounded-2xl h-12 focus:ring-4 focus:ring-primary/5 focus:bg-base-100 transition-all text-xs font-bold uppercase px-5">
            ...
            <div class="p-6 bg-base-100 border-t border-base-200/60 flex items-center justify-end gap-3 shrink-0">
                <button type="button" @click="new_user_modal.close()" class="px-6 py-3 font-bold text-[10px] uppercase tracking-widest opacity-30 hover:opacity-100 transition-all">Cancelar</button>
                <button type="submit" class="px-10 py-4 rounded-2xl font-black bg-primary text-primary-content shadow-xl shadow-primary/20 hover:shadow-primary/40 transition-all active:scale-95 uppercase text-[10px] tracking-[0.2em]">Salvar Registro</button>
            </div>
        </form>
    </div>
    <form method="dialog" class="modal-backdrop backdrop-blur-md bg-base-content/20 transition-all duration-500">
        <button>close</button>
    </form>
</dialog>

{{-- MODAL EDITAR USUÁRIO --}}
<dialog id="edit_user_modal" class="modal modal-bottom sm:modal-middle transition-all duration-500" x-data="{ 
    ...
    if (avatar) {
        editPhotoPreview = (avatar.startsWith('http')) ? avatar : '/storage/' + avatar;
    } else {
        editPhotoPreview = null;
    }
    $el.showModal();
">
    <div class="modal-box p-0 max-w-2xl bg-base-100 rounded-3xl overflow-hidden flex flex-col max-h-[90vh] shadow-2xl border border-base-content/5 z-10 transform transition-all duration-300 scale-95 opacity-0 [.modal[open]_&]:scale-100 [.modal[open]_&]:opacity-100">
        
        {{-- Header Identico ao Cadastro --}}
        ...
        <form :action="'/admin/users/' + userId" method="POST" enctype="multipart/form-data" class="flex flex-col flex-1 overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto p-8 pt-6 space-y-8 custom-scrollbar scroll-smooth">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div class="form-control group">
                        <label class="label py-1 ml-1">
                            <span class="label-text font-black text-[9px] uppercase tracking-widest opacity-40 group-focus-within:text-primary transition-all">Nova Senha</span>
                        </label>
                        <input type="password" name="password" placeholder="Deixe em branco para manter"
                            class="w-full px-5 h-12 bg-base-200/50 border-none rounded-2xl focus:outline-none focus:ring-4 focus:ring-primary/5 focus:bg-base-100 transition-all text-sm placeholder:text-base-content/20" />
                    </div>
            ...
            <div class="p-6 bg-base-100 border-t border-base-200/60 flex items-center justify-end gap-3 shrink-0">
                <button type="button" @click="edit_user_modal.close()"
